...
-- Includes: Soft-delete support and updated_at triggers.
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS depotdirect.site_tanks (
  ...
  product_id integer NOT NULL REFERENCES depotdirect.products(id),
  ...
  deleted_at timestamptz,            -- Support for soft-delete
...
CREATE TABLE IF NOT EXISTS depotdirect.tank_readings (
  ...
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now(),
  deleted_at timestamptz             -- Record historical deletions
);
...
-- Soft-delete trigger for site_tanks
DROP TRIGGER IF EXISTS trg_soft_delete_site_tanks ON depotdirect.site_tanks;
CREATE TRIGGER trg_soft_delete_site_tanks
    BEFORE DELETE ON depotdirect.site_tanks
